...
function sortObj(data) {
  ...
  for (var i = 0; i < activities.length - 1; i++) {
    var transactionType = activities[i].transaction_type;

    if (!sortedData[transactionType])
      sortedData[transactionType] = [];

    sortedData[transactionType].push(activities[i]);
  }

  for (var transactionType in sortedData) {
    separateDate(sortedData[transactionType]);
  }
}
